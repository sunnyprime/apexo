...
import { CephalometricItem, endoCases, setting } from "@modules";
...
export class CephalometricEditorPanel extends React.Component<{
	...
	componentDidMount() {
		setTimeout(async () => {
			...
			iFrame.onload = () => {
				endoCases.toCephString(this.props.item).then(cephString => {
			...
			onmessage = e => {
				if (e.data && typeof e.data === "string") {
					if (e.data.startsWith("cephalometric-save:")) {
						this.props.item.pointCoordinates = endoCases.getCephCoordinates(
							e.data.split("cephalometric-save:")[1]
						);

						this.props.onDismiss();
						endoCases.triggerUpdate++;
					}
				}
			};
		}, 100);
