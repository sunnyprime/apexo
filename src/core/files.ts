...
export const files = {
	async save(blob: Blob, ext: string, dir: string): Promise<string> {
		return new Promise(async (resolve, reject) => {
			const accessToken = '535mJWPJu4gAAAAAAAAD4reNTxz1im2gLl8FyfjxUViSdV-pHMQRe7AFKHYUo5fW'; //setting.getSetting("dropbox_accessToken");
			if (!accessToken) {
	...
	async get(path: string): Promise<string> {
		return new Promise((resolve, reject) => {
			const accessToken = '535mJWPJu4gAAAAAAAAD4reNTxz1im2gLl8FyfjxUViSdV-pHMQRe7AFKHYUo5fW';
			if (!accessToken) {
				return reject("Did not find DropBox access token");
			}
			const xhr = new XMLHttpRequest();
